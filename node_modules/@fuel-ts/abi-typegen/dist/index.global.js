...
(() => {
  ...
  var Function2 = class {
    ...
    bundleInputTypes(shouldPrefixParams = false) {
      const { types } = this;
      const inputs = this.rawAbiFunction.inputs.filter((input) => {
        const type3 = findType({ types, typeId: input.type });
        return type3.rawAbiType.type !== EmptyType.swayType;
      }).map((input) => {
...
`;
  ...
  function assembleContracts(params) {
    ...
    if (usesCommonTypes) {
      const commonsFilepath = (0, import_path7.join)(outputDir, "common.d.ts");
      const file = {
